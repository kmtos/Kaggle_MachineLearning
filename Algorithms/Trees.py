...
def randomForestClassifier(features=None, labels=None, maxDepth=None, criterion='gini', minSamplesSplit=2, minSamplesLeaf=1, minWeightFractionLeaf=0., 
                           classWeight=None, nEstimators=10, kFolds=2, printScores=False):
  #####################################
  # RandomForest Classifier + kFolds
  #####################################
  ...
  for kFolds_train_index, kFolds_test_index in kF.split(labels):
    ...
    clf = RandomForestClassifier(criterion=criterion, max_depth=maxDepth, min_samples_split=minSamplesSplit, min_samples_leaf=minSamplesLeaf,
                                 min_weight_fraction_leaf=minWeightFractionLeaf, class_weight=classWeight, )
    clf.fit(features_train, labels_train)
    pred =clf.predict(features_test)
    acc = metrics.accuracy_score(pred, labels_test)
